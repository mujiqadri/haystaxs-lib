-- Function: haystack_ui.load_querylog(text, text, text, text, integer)

-- DROP FUNCTION haystack_ui.load_querylog(text, text, text, text, integer);

CREATE OR REPLACE FUNCTION haystack_ui.load_querylog(haystackschema TEXT, userschema TEXT, qrylogtblname TEXT,
                                                     ext_tblname    TEXT, queryid INTEGER)
  RETURNS VOID AS
  $BODY$
  DECLARE
    mycommand  TEXT;
...
ALTER FUNCTION haystack_ui.load_querylog( TEXT, TEXT, TEXT, TEXT, INTEGER )
OWNER TO gpadmin;
